DeadworksApp.factory('GlobalService', ['$rootScope', '$window', '$timeout', '$http', '$filter', '$location', '$q', function($rootScope, $window, $timeout, $http, $filter, $location, $q){

  // redirect with # for old-ass Android browsers that can't hang
  (function(){
    if (Deadworks.prop.platform.IS_LEGACY_ANDROID && !window.location.hash){
  ...
  globalService.location = (function(){
    if (Deadworks.prop.platform.IS_LEGACY_IE){
